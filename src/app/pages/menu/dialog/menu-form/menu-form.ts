...
export class MenuForm {
  ...
  menuForm = form(this.menuModel, (schemaPath) => {
    ...
    required(schemaPath.target, { message: 'Target is required' });
  });
    ...
    this.#api
      ...
      .subscribe({
        next: (response: IHttpResponse) => {
          this.parentOptions.set(response.data?.list || []);
        },
      });
  }
  ...
  onSubmit(): void {
    ...
    this.#api.post<IHttpResponse>(URL, this.menuForm().value()).subscribe({
      next: () => {
        Swal.fire({
